    ...
    public class UpdateListingCaseRequestValidator: AbstractValidator<UpdateListingCaseRequest>
    {
        ...
        public UpdateListingCaseRequestValidator()
        {
            ...
            RuleFor(x => x.SaleCategory)
                .IsInEnum()
                .WithMessage("SaleCategory must be one of: ForSale, ForRent, Auction.");

            RuleFor(x => x.ListingCaseStatus)
                .IsInEnum()
                .WithMessage("ListingCaseStatus must be one of: Created, Pending, Delivered.");
        }
    }
}
